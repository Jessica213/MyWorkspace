...
| `ai-content.js` | AI 每日内容生成模块（智谱 GLM-4-Flash） |
| `supabase.min.js` | Supabase JS SDK（本地版，不依赖 CDN） |
...
配置后，每天自动生成新鲜的学习内容，不用再反复做那几道题了。

### 能生成什么
- 📖 **阅读长材料并复述**：每天一篇 300-500 字短文，附复述要点
- 🗣️ **英语口语练习**：每天一个新话题，附常用句型和参考范文
- 💡 **今日智慧锦囊**：每天一句 AI 生成的智慧短句

### 如何配置
1. 打开 https://open.bigmodel.cn/ ，注册账号
2. 进入「API 密钥」页面，创建一个新的 API Key
3. 打开成长工作台 → 右上角「云同步」→「智谱 AI 设置」
4. 粘贴 API Key → 保存
5. 保存后会自动生成今日内容，稍等几秒就好

### 使用说明
- **每天自动更新**：每天第一次打开应用，自动生成当天的新内容
- **手动换一篇**：阅读/口语页面点「换一篇」，智慧锦囊点「换一条」
- **免费额度**：GLM-4-Flash 是免费模型，每天调用量足够个人使用
- **离线可用**：生成过的内容存在本地，断网也能看
- **生成失败**：网络不好或 API 调用失败时，自动回退到本地题库

### 配置方式（二选一）
**方式 A：在应用内填写（推荐）**
- 点「云同步」→「智谱 AI 设置」→ 填入 API Key → 保存
- 配置存在浏览器本地

**方式 B：部署前预置**
- 打开 `config.js`，把 `zhipuApiKey` 填进去
- 保存后部署，所有打开的用户都自动有 AI 功能
